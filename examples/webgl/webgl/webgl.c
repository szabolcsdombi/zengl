...
typedef struct DefaultLoader {
    PyObject_HEAD
} DefaultLoader;

static PyTypeObject * Window_type;
static PyTypeObject * DefaultLoader_type;
...
static PyMethodDef DefaultLoader_methods[] = {
    {"load_opengl_function", (PyCFunction)load_opengl_function, METH_O},
    {NULL},
};

static PyType_Slot DefaultLoader_slots[] = {
    {Py_tp_methods, DefaultLoader_methods},
    {Py_tp_dealloc, default_dealloc},
    {0},
};

static PyType_Spec Window_spec = {"webgl.Window", sizeof(Window), 0, Py_TPFLAGS_DEFAULT, Window_slots};
static PyType_Spec DefaultLoader_spec = {"webgl.DefaultLoader", sizeof(DefaultLoader), 0, Py_TPFLAGS_DEFAULT, DefaultLoader_slots};
...
extern PyObject * PyInit_webgl() {
    ...
    DefaultLoader_type = (PyTypeObject *)PyType_FromSpec(&DefaultLoader_spec);

    Window * window = PyObject_New(Window, Window_type);
    PyModule_AddObject(module, "window", (PyObject *)window);

    PyObject * zengl_helper = PyImport_ImportModule("_zengl");
    PyObject_SetAttrString(zengl_helper, "DefaultLoader", (PyObject *)DefaultLoader_type);
