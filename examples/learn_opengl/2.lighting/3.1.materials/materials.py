...
from utils import Camera, Window, read_file, read_vertices, set_uniform_float, set_uniform_glm
...
lighting_shader = ctx.pipeline(
    vertex_shader=read_file('3.1.materials.vs'),
    fragment_shader=read_file('3.1.materials.fs'),
    uniforms='all',
...
light_cube_shader = ctx.pipeline(
    vertex_shader=read_file('3.1.light_cube.vs'),
    fragment_shader=read_file('3.1.light_cube.fs'),
    uniforms='all',
    framebuffer=[image, depth],
    topology='triangles',
    vertex_buffers=zengl.bind(vertex_buffer, '3f 3f', 0, -1),
...
while window.update():
    ...
    set_uniform_glm(lighting_shader, 'material.ambient', glm.vec3(1.0, 0.5, 0.31))
    set_uniform_glm(lighting_shader, 'material.diffuse', glm.vec3(1.0, 0.5, 0.31))
    set_uniform_glm(lighting_shader, 'material.specular', glm.vec3(0.5, 0.5, 0.5))
