...
lighting_shader = ctx.pipeline(
    vertex_shader=read_file('2.1.basic_lighting.vs'),
    fragment_shader=read_file('2.1.basic_lighting.fs'),
    uniforms='all',
...
light_cube_shader = ctx.pipeline(
    vertex_shader=read_file('2.1.light_cube.vs'),
    fragment_shader=read_file('2.1.light_cube.fs'),
    uniforms='all',
    framebuffer=[image, depth],
    topology='triangles',
    vertex_buffers=zengl.bind(vertex_buffer, '3f 3f', 0, -1),
