...
lighting_shader = ctx.pipeline(
    vertex_shader=read_file('5.1.light_casters.vs'),
    fragment_shader=read_file('5.1.light_casters.fs'),
    resources=[
        {
            'type': 'sampler',
            'binding': 0,
...
while window.update():
    ...
    set_uniform_glm(lighting_shader, 'light.direction', glm.vec3(-0.2, -1.0, -0.3))
