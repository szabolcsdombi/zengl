...
lighting_shader = ctx.pipeline(
    vertex_shader=read_file('5.4.light_casters.vs'),
    fragment_shader=read_file('5.4.light_casters.fs'),
    layout=[
        {
...
while window.update():
    ...
    set_uniform_glm(lighting_shader, 'light.position', camera.position)
    set_uniform_glm(lighting_shader, 'light.direction', camera.target - camera.position)
    set_uniform_float(lighting_shader, 'light.cutOff', glm.cos(glm.radians(12.5)))
    set_uniform_float(lighting_shader, 'light.outerCutOff', glm.cos(glm.radians(17.5)))
    set_uniform_glm(lighting_shader, 'light.ambient', glm.vec3(0.1, 0.1, 0.1))
    set_uniform_glm(lighting_shader, 'light.diffuse', glm.vec3(0.8, 0.8, 0.8))
